...
test('API exposes read-only live comparison route', async () => {
  ...
  const url = await app.listen(); const response = await fetch(`${url}/api/careers`); assert.equal(response.status, 200); assert.deepEqual(await response.json(), []); app.close(); db.close();
});
