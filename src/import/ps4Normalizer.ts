...
import type { CareerFacts, CompetitionProgress, FixtureEvidence, LeagueFacts, LeagueStanding, SeasonRecord, TransferEvent } from '../domain/careerFacts.js';
...
export interface NormalizationContext {
  ...
  readonly fixtureEvidence?: FixtureEvidence | null;
  readonly nationalityResolver?: (id: number | null) => string | null;
}

const directAttributes = ['acceleration', 'sprintspeed', 'finishing', 'shotpower', 'longshots', 'volleys', 'penalties', 'positioning', 'vision', 'crossing', 'freekickaccuracy', 'shortpassing', 'longpassing', 'curve', 'agility', 'balance', 'reactions', 'ballcontrol', 'dribbling', 'composure', 'interceptions', 'headingaccuracy', 'defensiveawareness', 'standingtackle', 'slidingtackle', 'jumping', 'stamina', 'strength', 'gkdiving', 'gkhandling', 'gkkicking', 'gkpositioning', 'gkreflexes'];
...
export function normalizePs4Save(parsed: unknown, context: NormalizationContext): SnapshotCandidate {
  ...
  for (const link of links) {
    ...
    const attributes = Object.fromEntries(directAttributes.flatMap((field) => { const value = number(player, field); return value === null ? [] : [[field, value]]; }));
    playerStates.push({ playerId, name: context.nameResolver(playerId), preferredPositions, squadPosition: number(link, 'position'), overall: number(player, 'overallrating'), potential: number(player, 'potential'), birthdate: number(player, 'birthdate'), height: number(player, 'height'), weight: number(player, 'weight'), nationality: number(player, 'nationality'), nationalityName: context.nationalityResolver?.(number(player, 'nationality')) ?? null, preferredFoot: number(player, 'preferredfoot'), attributes, contractUntil: number(player, 'contractvaliduntil'), wage: number(contract, 'wage'), contractDurationMonths: number(contract, 'duration_months'), contractStatus: number(contract, 'contract_status'), playerRole: number(contract, 'playerrole'), contractDate: number(contract, 'contract_date'), lastStatusChangeDate: number(contract, 'last_status_change_date'), signOnBonus: number(contract, 'signon_bonus'), performanceBonusValue: number(contract, 'performancebonusvalue'), jerseyNumber: number(link, 'jerseynumber'), leagueAppearances: number(link, 'leagueappearances'), leagueGoals: number(link, 'leaguegoals'), evidence: evidenceFor(['overall', 'potential', 'preferredPositions', 'height', 'contractUntil', 'squadPosition', 'wage']) });
  }
