...
class ParsedCareer:
    ...
    name_coverage: str
    raw_output: str
    squad_players: tuple[dict, ...] = ()
    academy_player_records: tuple[dict, ...] = ()
...
def parse_json_output(output: str) -> ParsedCareer:
    ...
    return ParsedCareer(data["manager_name"], data["club_name"], str(data["season"]), data["estimated_date"], data["databases"], tuple(data["table_counts"]), data["senior_players"], data["academy_players"], "", output, tuple(data.get("squad_players", [])), tuple(data.get("academy_player_records", [])))
